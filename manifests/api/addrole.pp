...
define cloudera::api::addrole (
  ...
  exec { "add role for service $cdh_service_name":
    command => "/usr/bin/curl -H 'Content-Type: application/json' -u $cm_api_user:$cm_api_password -XPOST \"http://$cm_api_host:$cm_api_port/api/v13/clusters/$cdh_cluster_name/services/$cdh_service_name/roles\" -d @$cdh_service_name-roles.json > $cdh_metadata_dir/$cdh_service_name-roles.json.output",
    cwd     => "/tmp",

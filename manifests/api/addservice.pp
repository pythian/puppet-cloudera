...
define cloudera::api::addservice (
  ...
  exec { "add service $cdh_service_name":
    command => "/usr/bin/curl -H 'Content-Type: application/json' -u $cm_api_user:$cm_api_pass -XPOST \"http://$cm_api_host:$cm_api_port/api/v13/clusters/$cdh_cluster_name/services\" -d @$cdh_service_name.json > $cdh_metadata_dir/$cdh_service_name.json.output",
    cwd     => "/tmp",

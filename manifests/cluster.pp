    ...
    if $cdh_cluster_ha == 0 {
      ...
      if $cm_db_embedded == 1 {
        ...
        if $cm_db_rds == 0 {
          ...
          class { 'mysql::client': }
          mysql_database{ "$actmon_db_name": require => Class['::mysql::client'], ensure => present }
          ...
          #mysql_user{ "$cm_db_user@%": ensure => present, password_hash => mysql_password("$cm_db_pass"), require => Class['::mysql::client'], ensure => present}

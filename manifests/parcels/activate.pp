define cloudera::parcels::activate (
  ...
  exec { "parcels-activate-$parcels_product":
    command => "/usr/bin/curl -u $cm_api_user:$cm_api_password -XPOST \"http://$cm_api_host:$cm_api_port/api/v13/clusters/$cdh_cluster_name/parcels/products/$parcels_product/versions/$parcels_version/commands/activate\" > $cdh_metadata_dir/parcels-activate-$parcels_product.json.output",
    cwd     => "/tmp",
